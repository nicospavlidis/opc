...
\title{Optimal Projections for Clustering: An open source MATLAB and GNU Octave library for Dimensionality Reduction for Clustering}
...
is an open source MATLAB and GNU Octave library that implements
...
%
Contributors, or users that prefer a local copy of this
can use the 
%
\href{https://www.artefact.tk/software/matlab/m2html/}{M2HTML}
%
documentation system to automatically generate an HTML function
reference for OPC.
...
%
%Using {\tt help} from the MATLAB or GNU Octave terminal
...
Note that the default initialisation of {\tt kmeans} in both MATLAB and GNU
...
>> km = kmeans(X,10);
>> % Spectral clustering
>> % (In GNU Octave the pdist function can fail for this dataset size)
>> sc = spclustNJW(X,10);
\end{verbatim}
...
>> % GNU Octave can work even if it failed on the original dataset)
...
%The fact that $k$-means is not guaranteed to identify the globally optimal
%solution, and the sensitivity of the {\tt kmeans} function to random
%initialisation can produce very different outputs over successive executions
%of this algorithm.
...
\begin{htmlonly}
%
\includegraphics[scale=0.7]{figures/mddc1.png}} 
%
\end{htmlonly}
%
\caption{Binary partition at node 2 of cluster hierarchy produced by MDDC algorithm.}
%
\label{fig:mddc1}
\end{figure}
%
...
MATLAB and GNU Octave. To modify the clustering model the
...
%
%it bi-partitions a leaf node with the separator estimated using all the
%parameter values specified during the first call
%
...
\begin{center}
%
\subfigure[Original space]{\includegraphics[scale=0.22]{figures/kpca1.png}} \hspace{0.3cm}
%
\subfigure[Feature space]{\includegraphics[scale=0.22]{figures/kpca2.png}} 
%
\end{center}
\end{latexonly}
%
...
\end{htmlonly}
%
\caption{Clustering obtained by hyperplane separators in the original, Euclidean space and in the kernel
