...
%- Add some new blocks in order to have the desire theme 
%- ( all functions under subdirectories)
%- Dimitris Kostaras
...
tpl = set(tpl,'block','TPL_MASTER','row-m','rows-m');
tpl = set(tpl, 'block', 'rowdir', 'row-m', 'rows-m');
...
for i=2:length(mdir)
	...
	tpl = set(tpl,'var','DIR_NAME', path{end});
	
	%- New block to provide the functions)
	...
	for j=1:length(mdirs)
		if strcmp(mdirs{j},mdir{i})
			...
			tpl = set(tpl,'var', 'FILE_SOURCE', fullurl(mdir{i},[names{j} options.extension]));
			if any(ismex(j,:))
...
tpl = set(tpl,'block','TPL_MFILE','rowdir','rowdirs');
tpl = set(tpl,'block','TPL_MFILE','row-m','rows-m');
tpl = set(tpl, 'block', 'rowdir', 'row-m', 'rows-m');
tpl = set(tpl,'block','row-m','mexfile','mex');
...
for i=2:length(mdir)
	...
	%- Dimitris Kostaras
	path = strsplit(mdir{i} ,'/');
	tpl = set(tpl,'var','DIR_NAME', path{2});
