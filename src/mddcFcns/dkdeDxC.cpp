...
// debugged
double dkdeDx(const double &x, const int &N, const double *proj, const double &h)
{
	const double PI = 3.141592654;
	double h2= 2*h*h;
	double y = 0.;
	for (int i=0; i < N; i++) {
		y += (proj[i] - x) * exp((proj[i]-x)*(x - proj[i])/h2);
	}
	return y/(N*sqrt(2.*PI)*h*h*h);
}
